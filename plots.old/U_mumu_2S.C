...
void U_mumu_2S()
{
	...
	{
		if (*Ups_mumu_d1_pT > 0.2 && abs(*Ups_mumu_d1_eta) < 1 && *Ups_mumu_d2_pT > 0.2 && abs(*Ups_mumu_d2_eta) < 1 && *Ups_mumu_2S == 1)
		{
	...
	// fill canvases
	TCanvas* c2 = new TCanvas("c2", "#Upsilon(2S) in #mu^{+}#mu^{-}", 800, 600);

	TH1D *h2 = new TH1D("h2", "#Upsilon(2S) in #mu^{+}#mu^{-}", 4000, 0, 4);
	...
	c2->cd();
	c2->Print("../output.general/Ups_mumu_2S_500.png", "png");
}
