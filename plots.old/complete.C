...
void complete()
{
	...
	TFile *fileMB = TFile::Open("../data/PY_MinBias.root");
	TFile *fileUps_ee = TFile::Open("../data/PY_Upsilon_ee.root");
	TFile *fileUps_mm = TFile::Open("../data/PY_Upsilon_mumu.root");

	TTreeReader readMB("tree", fileMB);
	TTreeReader readUps_ee("tree", fileUps_ee);
	TTreeReader readUps_mm("tree", fileUps_mm);
	...
	TTreeReaderValue<Double_t> Ups_mm_nChar(readUps_mm, "nChar");
	TTreeReaderValue<Double_t> Ups_mm_pT(readUps_mm, "p_pT");
	TTreeReaderValue<Double_t> Ups_mm_d1_pT(readUps_mm, "d1_pT");
	TTreeReaderValue<Double_t> Ups_mm_d1_eta(readUps_mm, "d1_eta");
	TTreeReaderValue<Double_t> Ups_mm_d2_pT(readUps_mm, "d2_pT");
	TTreeReaderValue<Double_t> Ups_mm_d2_eta(readUps_mm, "d2_eta");
	TTreeReaderValue<Double_t> Ups_mm_1S(readUps_mm, "Ups1S");
	TTreeReaderValue<Double_t> Ups_mm_2S(readUps_mm, "Ups2S");

	// kamo vole multiplicitni distribuce vole
	TH1D* hist_MB = new TH1D("hist_MB", "", 80, 0, 80); 
	TH1D* hist_Ups_ee = new TH1D("hist_Ups_ee", "", 80, 0, 80); 
	TH1D* hist_Ups_mm = new TH1D("hist_Ups_mm", "", 80, 0, 80); 

	while(readMB.Next())
		hist_MB->Fill(*MB_nChar);
	readMB.Restart();

	while(readUps_ee.Next())
		hist_Ups_ee->Fill(*Ups_ee_nChar);
	readUps_ee.Restart();

	while(readUps_mm.Next())
		hist_Ups_mm->Fill(*Ups_mm_nChar);
	readUps_mm.Restart();

	// mrd distribuce na papir
	TCanvas *c_mult = new TCanvas("c_mult", "Multiplicity distributions", 800, 600);
	c_mult->SetLogy();

	hist_MB->SetLineColor(kRed);
	hist_Ups_ee->SetLineColor(kBlue);
	hist_Ups_mm->SetLineColor(kGreen);

	hist_MB->SetStats(kFALSE);
	hist_Ups_ee->SetStats(kFALSE);
	hist_Ups_mm->SetStats(kFALSE);

	hist_MB->GetYaxis()->SetTitle("Count");

	hist_MB->GetXaxis()->SetTitle("N_{ch}");

	TLegend *leg_mult = new TLegend(.15, .15, .4, .4);
	leg_mult->SetTextSize(0.035);
	leg_mult->SetHeader("p+p@500 GeV, |#eta| < 1");
	leg_mult->AddEntry(hist_MB, "Minimum Bias", "f");
	leg_mult->AddEntry(hist_Ups_ee, "Upsilon in e^{+}e^{-}", "f");
	leg_mult->AddEntry(hist_Ups_mm, "Upsilon in #mu^{+}#mu^{-}", "f"); 

	hist_MB->Draw("");
	hist_Ups_ee->Draw("same");
	hist_Ups_mm->Draw("same");

	leg_mult->Draw("same");

	c_mult->Print("../output/mult.png", "png");
	// nastavime binovani
	Int_t bins_own = 14;
	Int_t bins_STAR = 5;
	...
	for (int i = 0; i < bins_STAR; i++)
	{
		...
		cout << "bin " << i << ": " << corrUps_ee_1S_bin_STAR[i] << endl;
	...
	for (int i = 0; i < bins_own; i++)
		y_err_ee_all_own[i] = 0.1*corrUps_ee_all_bin_own[i];
	...
	leg_ee_all_own->AddEntry(gr_ee_all_own, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_all_own->Print("../output/all_own.png", "png");

	// Ups all STAR
	Double_t y_err_ee_all_STAR[bins_STAR];
	for (int i = 0; i < bins_STAR; i++)
		y_err_ee_all_STAR[i] = 0.1*corrUps_ee_all_bin_STAR[i];
	...
	leg_ee_all_STAR->AddEntry(gr_ee_all_STAR, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_all_STAR->Print("../output/all_STAR.png", "png");
	...
	for (int i = 0; i < bins_own; i++)
		y_err_ee_1S_own[i] = 0.1*corrUps_ee_1S_bin_own[i];
	...
	leg_ee_1S_own->AddEntry(gr_ee_1S_own, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_1S_own->Print("../output/1S_own.png", "png");

	// Ups 1S STAR
	Double_t y_err_ee_1S_STAR[bins_STAR];
	for (int i = 0; i < bins_STAR; i++)
	{
		y_err_ee_1S_STAR[i] = 0.1*corrUps_ee_1S_bin_STAR[i];
		cout << "err" << i << " " << 10*y_err_ee_1S_STAR[i] << endl;
	}
	...
	leg_ee_1S_STAR->AddEntry(gr_ee_1S_STAR, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_1S_STAR->Print("../output/1S_STAR.png", "png");
	...
	for (int i = 0; i < bins_own; i++)
		y_err_ee_1S_pT_own[i] = 0.1*corrUps_ee_1S_pT_bin_own[i];
	...
	leg_ee_1S_pT_own->AddEntry(gr_ee_1S_pT_own, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_1S_pT_own->Print("../output/1S_pT_own.png", "png");

	// Ups 1S pT STAR
	Double_t y_err_ee_1S_pT_STAR[bins_STAR];
	for (int i = 0; i < bins_STAR; i++)
		y_err_ee_1S_pT_STAR[i] = 0.1*corrUps_ee_1S_pT_bin_STAR[i];
	...
	leg_ee_1S_pT_STAR->AddEntry(gr_ee_1S_pT_STAR, "PYTHIA 8 STAR HF Tune", "f");
	...
	c_ee_1S_pT_STAR->Print("../output/1S_pT_STAR.png", "png");

	// Ups 2S own
	Double_t y_err_ee_2S_own[bins_own];
	for (int i = 0; i < bins_own; i++)
		y_err_ee_2S_own[i] = 0.1*corrUps_ee_2S_bin_own[i];
	...
	leg_ee_2S_own->AddEntry(hist_1S_own, "PYTHIA 8 STAR HF Tune #Upsilon(1S)", "p");
	leg_ee_2S_own->AddEntry(hist_2S_own, "PYTHIA 8 STAR HF Tune #Upsilon(2S)", "p");

	hist_1S_own->Draw("P E1");
	hist_2S_own->Draw("same P E1");
	leg_ee_2S_own->Draw("same");
	c_ee_2S_own->Print("../output/2S_own.png", "png");

	// Ups 2S STAR
	Double_t y_err_ee_2S_STAR[bins_STAR];
	for (int i = 0; i < bins_STAR; i++)
		y_err_ee_2S_STAR[i] = 0.1*corrUps_ee_2S_bin_STAR[i];

	TCanvas* c_ee_2S_STAR = new TCanvas("c_ee_2S_STAR", "#Upsilon(2S+2S) in e^{+}e^{-}", 800, 600);


	c_ee_2S_STAR->Print("../output/2S_STAR.png", "png");
}
