...
void GetErrorHiLo(TGraphErrors *gr, TGraphErrors *&gr_lo, TGraphErrors *&gr_hi);
...
void GetErrorHiLo(TGraphErrors *gr, TGraphErrors *&gr_lo, TGraphErrors *&gr_hi)
{
	...
	gr_lo = (TGraphErrors *)gr->Clone(Form("%s_lo", gr->GetName()));
	gr_hi = (TGraphErrors *)gr->Clone(Form("%s_hi", gr->GetName()));
	...
	for (int i = 0; i < np; ++i) {
		...
		gr_lo->SetPoint(i, x, y-eyh);
		gr_lo->SetPointError(i, 0.0, 0.0);
		gr_hi->SetPoint(i, x, y+eyh);
		gr_hi->SetPointError(i, 0.0, 0.0);
