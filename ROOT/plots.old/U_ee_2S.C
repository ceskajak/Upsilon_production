...
void U_ee_2S()
{
	...
	{
		if (*Ups_ee_d1_pT > 0.2 && abs(*Ups_ee_d1_eta) < 1 && *Ups_ee_d2_pT > 0.2 && abs(*Ups_ee_d2_eta) < 1 && *Ups_ee_2S == 1)
		{
	...
	// fill canvases
	TCanvas* c1 = new TCanvas("c1", "#Upsilon(2S) in e^{+}e^{-}", 800, 600);

	TH1D *h1 = new TH1D("h1", "#Upsilon(2S) in e^{+}e^{-}", 4000, 0, 4);
	...
	*/
	...
	c1->cd();
	c1->Print("../output.general/Ups_ee_2S_500.png", "png");
}
